...
class PINN_PDESolver():
    ...
    def loss_fn(self, X, u):
        ...
        # self.phi_0_b = np.array(phi_0_b, dtype=DTYPE)
        loss += phi_0_b
    ...
    def solve_with_TFoptimizer(self, optimizer, X, u, 
                               N=None, min_loss=None, timeout=None, modify_lr=False):
        ...
        If modify_lr is True, learning rate will change to piecewise constant decay
        after 1000 iterations with given optimizer. Otherwise, modify_lr can be either
        float or integer which represents minimum loss or maximum iterations reached
        respectively."""
        ...
        try:
            initial_learning_rate = optimizer._decayed_lr(tf.float32)                
            print('Initial Learning Rate = {:10.5e}'.format(initial_learning_rate))
        except TypeError as e:
            pass
        
        
        def set_modify_lr(i_stop, i_stop_count):            
            # if(self.current_loss < 10e-1 or self.iter >= 1000):
            #     if (i_stop == 0):
            #         i_stop = self.iter
            #         optimizer.lr = optimizer._decayed_lr(tf.float32)
            #         print('Current Learning Rate = {:10.5e}'.format(optimizer.lr))
            #     elif (i_stop_count % 2000 == 0 and 0 < i_stop_count <= 4000):
            #         optimizer.lr /= 2
            #         print('Current Learning Rate = {:10.5e}'.format(optimizer.lr))                
            #     return i_stop, i_stop_count + 1
            # return 0, 0

            if (isinstance(modify_lr, float) and self.current_loss <= modify_lr) or\
                (isinstance(modify_lr, int) and modify_lr is not True and self.iter >= modify_lr):
                current_lr = optimizer._decayed_lr(tf.float32)                
                optimizer.lr = tf.keras.optimizers.schedules.PiecewiseConstantDecay([2000],[current_lr,current_lr/2])

        # i_stop = 0; i_stop_count = 0
        # if modify_lr is True:
        #     modify_lr = 1000
        ...
        if N is not None:
            while True:        
                ...
                # if modify_lr is not False:
                #     # i_stop, i_stop_count = set_modify_lr(i_stop, i_stop_count)
                #     set_modify_lr(i_stop, i_stop_count)
...
class Hybrid_IdentificationSolver(PINN_PDESolver):
    def solve(self, optim_fwd, optim_param, get_r_param, X, u, N_param=None, min_loss_param=None, timeout_param=None, modified=False, **kwargs):
        self.changed = False
        self.lambd_list = []
        self.solve_with_TFoptimizer(optim_fwd, X, u, **kwargs)
        ...
        if modified is True:
        ...
        elif modified2 is True:
            def get_grad_param(self, X, u):
                with tf.GradientTape(persistent=True) as tape:
                    # This tape is for derivatives with
                    # respect to trainable variables
                ...
                g = tape.gradient(loss, self.model.lambd)
                del tape
            ...
            @tf.function
            def train_step_param():
                loss, grad_theta = get_grad_param(self, X, u)
